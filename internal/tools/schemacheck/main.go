...
func main() {
	...
	if *schemaPath == "" || *inputPath == "" {
		fmt.Fprintln(os.Stderr, "usage: schemacheck --schema <schema.json> --input <input.json>")
		os.Exit(2)
	}
...
func run(schemaPath, inputPath string) error {
	...
	got, ok := payload["schema_version"].(string)
	if !ok {
		return errors.New("schema_version must be a string")
	}
	if got != s.SchemaVersion {
